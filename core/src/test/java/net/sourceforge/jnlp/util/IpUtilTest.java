...
public class IpUtilTest {
    ...
    @Test
    public void testIsHostIP4Address() {
        Assert.assertTrue(IpUtil.isHostAnIPAddress("192.168.0.1"));
        Assert.assertTrue(IpUtil.isHostAnIPAddress("192.168.0.1:80"));
        Assert.assertTrue(IpUtil.isHostAnIPAddress("123.234.255.255"));
        Assert.assertFalse(IpUtil.isHostAnIPAddress("-123.234.255.255"));
        Assert.assertFalse(IpUtil.isHostAnIPAddress("123.234.257.255"));
        Assert.assertTrue(IpUtil.isHostAnIPAddress((String) null));
        Assert.assertTrue(IpUtil.isHostAnIPAddress("123.*.156.234"));
        Assert.assertFalse(IpUtil.isHostAnIPAddress("www.google.com"));
        Assert.assertFalse(IpUtil.isHostAnIPAddress("www.123.com"));
        Assert.assertFalse(IpUtil.isHostAnIPAddress("www.123.234"));
    }

    @Test
    public void testValidateIPHost() {
        try {
            IpUtil.validateIPHost("www.google.234");
            assertTrue(true);
        } catch (HostNameException e) {
            Assert.fail();
        }
        try {
            IpUtil.validateIPHost("www.google.com");
            assertTrue(true);
        } catch (HostNameException e) {
            Assert.fail();
        }
        try {
            IpUtil.validateIPHost("123.234.25*.234");
            Assert.fail();
        } catch (HostNameException e) {
            assertTrue(true);
        }
        try {
            IpUtil.validateIPHost("23.234.256.234");
            Assert.fail();
        } catch (HostNameException e) {
            assertTrue(true);
        }
        try {
            IpUtil.validateIPHost("23.-234.255.234");
            Assert.fail();
        } catch (HostNameException e) {
            assertTrue(true);
        }
