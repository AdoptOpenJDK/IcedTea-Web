...
public class WebStartProxySelector extends ProxySelector {
    ...
    public WebStartProxySelector(final DeploymentConfiguration config, final ExecutorService ioExecutor, final BiConsumer<String, Throwable> errorHandler) {
        this.proxyProvider = createProvider(config, ioExecutor);
        this.errorHandler = errorHandler;
    }

    private ProxyProvider createProvider(final DeploymentConfiguration config, final ExecutorService ioExecutor) {
        ...
        try {
            ...
            return providerType.createProvider(config, ioExecutor);
        } catch (final Exception e) {
            errorHandler.accept(Translator.getInstance().translate("proxy.error.creationFailed"), e);
    ...
    public List<Proxy> select(final URI uri) {
        try {
            ...
            errorHandler.accept(Translator.getInstance().translate("proxy.error.selectionFailed", uri), e);
            return JNLPRuntime.exit(-1);
        }
    }
    ...
    public void connectFailed(final URI uri, final SocketAddress proxyAddress, final IOException ioe) {
        ...
        if (proxyAddresses.get(proxyAddresses.size() - 1).equals(currentProxyAddress)) {
            errorHandler.accept(Translator.getInstance().translate("proxy.error.connectionFailed", currentProxyAddress, uri), ioe);
            JNLPRuntime.exit(-1);
        }
    }
